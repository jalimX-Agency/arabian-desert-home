...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = await db.blogPost.findUnique({
    where: { slug },
    select: { title: true, titleEs: true, excerpt: true, excerptEs: true, image: true },
  });
  if (!post) return {};
  const title = post.titleEs || post.title;
  const excerpt = post.excerptEs || post.excerpt;
  ...
  const metaTitle = title.length <= 30 ? `${title} | Blog de Arabian Desert Home` : title;
  return {
    title: metaTitle,
    ...
    locale: "es_ES",
      title,
      description: excerpt || title,
      url: `https://www.arabiandeserthome.ma/es/blog/${slug}`,
    ...
    alternates: esAlternates(`/blog/${slug}`),
  };
}

export default async function SpanishBlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await db.blogPost.findUnique({ where: { slug } });
  if (!post) notFound();

  // Related articles: same category first, then most recent others
  const relatedSelect = { id: true, title: true, titleEn: true, titleEs: true, slug: true, image: true, category: true } as const;
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: "https://www.arabiandeserthome.ma/es/blog" },
      { "@type": "ListItem", position: 3, name: title, item: `https://www.arabiandeserthome.ma/es/blog/${slug}` },
    ],
  });
