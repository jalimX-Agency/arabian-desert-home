...
import { CalendarCheck, Mail, BedDouble, Clock, CheckCircle, XCircle } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="p-8 space-y-8">
      <div>
        <h1 className="text-2xl font-semibold text-white mb-1">Tableau de bord</h1>
        <p className="text-sm text-white/40">Bienvenue dans l&apos;espace administration</p>
      </div>

      {dbError && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-sm px-4 py-3 rounded-xl">
          ⚠ {dbError} — les données affichées peuvent être incomplètes.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          ...
          {stats.recentBookings.length === 0 ? (
            <p className="px-5 py-8 text-sm text-white/30 text-center">Aucune réservation</p>
          ) : (
            <div className="divide-y divide-white/5">
              {stats.recentBookings.map((b) => (
                <div key={b.id} className="px-5 py-3.5 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-sm text-white truncate">{b.firstName} {b.lastName}</p>
                    <p className="text-xs text-white/40 truncate">{b.suite.name} · {format(new Date(b.checkIn), "d MMM yyyy", { locale: fr })}</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-sm text-amber-400">{b.totalAmount} MAD</span>
                    <span className={`text-[10px] px-2 py-1 rounded-full uppercase tracking-widest ${statusColors[b.status] ?? "bg-white/10 text-white/60"}`}>
                      {b.status}
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/10">
            <h2 className="text-sm font-semibold text-white">Derniers messages</h2>
          </div>
          {stats.recentMessages.length === 0 ? (
            <p className="px-5 py-8 text-sm text-white/30 text-center">Aucun message</p>
          ) : (
            <div className="divide-y divide-white/5">
              {stats.recentMessages.map((m) => (
                <div key={m.id} className="px-5 py-3.5 flex items-center justify-between gap-4">
                  <div className="min-w-0 flex items-center gap-3">
                    {!m.read && <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" />}
                    <div className="min-w-0">
                      <p className={`text-sm truncate ${m.read ? "text-white/60" : "text-white"}`}>{m.name}</p>
                      <p className="text-xs text-white/40 truncate">{m.subject}</p>
                    </div>
                  </div>
                  <div className="shrink-0 flex items-center gap-2">
                    {m.read ? <XCircle className="w-3.5 h-3.5 text-white/20" /> : <Clock className="w-3.5 h-3.5 text-amber-400/60" />}
                    <span className="text-xs text-white/30">{format(new Date(m.createdAt), "d MMM", { locale: fr })}</span>
                  </div>
                </div>
              ))}
            </div>
