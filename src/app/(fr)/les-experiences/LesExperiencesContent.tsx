...
export function LesExperiencesContent({ experiences }: { experiences: Activity[] }) {
  ...
  return (
    <>
      ...
      <section ref={heroRef} className="relative">
        <div className="relative h-[40vh] md:h-[50vh] w-full overflow-hidden">
          <img
            ...
            alt="Expériences complètes au désert d'Agafay"
      ...
      {/* ── Experiences List ── */}
      <section ref={listRef} className="py-20 md:py-28 px-6 md:px-10 relative">
        <div className="absolute inset-0 pattern-organic opacity-50 pointer-events-none" />

        <div className="max-w-7xl mx-auto relative z-10">
          <motion.span variants={revealUp} initial="hidden" animate={listInView ? "visible" : "hidden"} custom={0.1} className="luxury-label text-amber block mb-4">
            {t("experiencesPage.introLabel")}
          </motion.span>
          <motion.h2 variants={revealUp} initial="hidden" animate={listInView ? "visible" : "hidden"} custom={0.2} className="heading-display text-3xl md:text-5xl mb-4">
            {t("experiencesPage.introTitle1")}
            <br />
            <span className="italic text-amber">{t("experiencesPage.introTitle2")}</span>
          </motion.h2>
          <motion.div variants={fadeIn} initial="hidden" animate={listInView ? "visible" : "hidden"} custom={0.25} className="divider-accent max-w-xs mb-6" />
          <motion.p variants={revealUp} initial="hidden" animate={listInView ? "visible" : "hidden"} custom={0.3} className="body-editorial text-muted-foreground max-w-2xl mb-16">
            {t("experiencesPage.introText")}
          </motion.p>
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                  ...
                  <div className="lg:col-span-7 p-8 md:p-10 flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-4 flex-wrap">
                      ...
                      {activity.transportIncluded && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground body-editorial">{t("experiencesPage.transportIncluded")}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mb-6">
                      <span className="luxury-label text-amber block mb-3">{t("experiencesPage.includes")}</span>
                        ...
                        <div>
                          ...
                          <div className="flex items-baseline gap-1.5">
                            ...
                            <span className="text-sm text-muted-foreground body-editorial">
                              {activity.currency} {t("experiencesPage.perPerson")}
                            </span>
                          </div>
                        </div>
                        <Link
      ...
      <CTASection
        label={t("experiencesPage.ctaLabel")}
        title={t("experiencesPage.ctaTitle")}
        buttonText={t("experiencesPage.ctaButton")}
