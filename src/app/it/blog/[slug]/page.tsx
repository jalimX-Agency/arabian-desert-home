...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = await db.blogPost.findUnique({
    where: { slug },
    select: { title: true, titleIt: true, excerpt: true, excerptIt: true, image: true },
  });
  if (!post) return {};
  ...
  return {
    ...
    openGraph: {
      title,
      description: excerpt || title,
      url: `https://www.arabiandeserthome.ma/it/blog/${slug}`,
    ...
    alternates: itAlternates(`/blog/${slug}`),
  };
}

export default async function ItalianBlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await db.blogPost.findUnique({ where: { slug } });
  if (!post) notFound();

  // Related articles: same category first, then most recent others
  const relatedSelect = { id: true, title: true, titleEn: true, titleIt: true, slug: true, image: true, category: true } as const;
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: "https://www.arabiandeserthome.ma/it/blog" },
      { "@type": "ListItem", position: 3, name: title, item: `https://www.arabiandeserthome.ma/it/blog/${slug}` },
    ],
  });
